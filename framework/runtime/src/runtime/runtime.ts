  ...
  implements Context<State>
{
  ...
  private inner: RuntimeInner<Agents, State>;
  ...
  private resolveSync<T extends Runnable>(idOrRunnable: string | RunnableDefinition | T): T {
    ...
    if (!definition) {
      ...
      if (idOrRunnable instanceof Runnable) definition = idOrRunnable.definition;
...
class RuntimeInner<Agents extends { [name: string]: Runnable } = {}, State extends ContextState = ContextState> {
  constructor(public options: RuntimeOptions<Agents, State>) {
    ...
    this.container.register('blocklet_api_agent', { useClass: BlockletAPIAgent });
  ...
  runnableDefinitions: OrderedRecord<RunnableDefinition> = OrderedRecord.fromArray([]);
